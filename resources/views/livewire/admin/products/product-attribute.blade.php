      ...
      <div class="my-4 flex w-full flex-col items-center justify-between sm:flex-row">
        ...
        <div class="flex w-full items-center justify-between gap-x-3 sm:w-auto">
          <button wire:click.prevent="deleteSelected"
            ...
            Delete All {{ count($selectedProductAttribute) }}
          </button>
        </div>
      </div>
      ...
      <div
        ...
        <table style="width: 900px;" class="min-w-full">
          <thead>
            <tr>
              <th class="table-head">
                <input wire:model="selectAllProductAttributes" id="checkAllProduct" type="checkbox" class="checkbox">
              </th>
              ...
              <th wire:click.prevent="sortBy('created_at')" class="table-head">
                Ngày tạo
                <x-sort-arrow name="name" :sortColumnName="$sortColumnName" :sortDirection="$sortDirection" />
              </th>
              <th
                ...
                <tr wire:loading.class.delay="opacity-50 cursor-wait">
                  <td class="table-body">
                    <div class="flex items-center">
                      <input wire:model="selectedProductAttribute" value="{{ $attribute->id }}" type="checkbox" class="checkbox">
                    </div>
                  </td>
                  <td class="table-body">
                    <div class="line-clamp-2 leading-5 text-gray-900">{{ $attribute->name }}</div>
                  </td>
                  <td class="table-body">
                    <div class="line-clamp-2 leading-5 text-gray-900">{{ $attribute->created_at }}</div>
                  </td>
                  <td class="whitespace-no-wrap right justify-content-end border-b border-gray-200 px-6 py-4 text-right">
                    <div class="flex">
                      <a data-toggle="tooltip" data-placement="bottom"
                        ...
                        title="Sửa" href="{{ route('admin.attributes.edit', $attribute->id) }}" id="btLeft">
                        <i class="fas fa-edit" title="Sửa"></i>
                      </a>
                      <button wire:click="deleteSelectedAttribute({{ $attribute->id }})" type="button" id="deleteProductBtn"
                        title="Xóa" data-toggle="tooltip" data-placement="bottom"
                        data-modal-target="deleteAttributeModal" data-modal-toggle="deleteAttributeModal"
        ...
        @include('livewire.admin.products.modal-delete-attribute')
      </div>
      <div class="d-flex align-items-center justify-content-between mb-5 mt-5">
        {{-- {{ $attributes->links('livewire.custom-pagination') }} --}}
      </div>
    </div>
  </div>
