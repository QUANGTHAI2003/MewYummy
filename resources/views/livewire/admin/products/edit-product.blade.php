<form enctype="multipart/form-data" wire:submit.prevent="updateProduct">
    ...
    <div class="overflow-hidden">
      ...
      @dump($attribute_values)
      @dump($allAttributeValues) --}}
              ...
              <div class="grid grid-cols-3 items-end gap-6 py-3 md:grid-cols-6">
                @foreach ($inputs as $key => $value)
                  <div style="grid-column: span 5 / span 3 !important;">
                    ...
                    <input type="text" wire:model.debounce.500ms="attribute_values.{{ $value }}" wire:keyup="attributeValue"
          ...
          <div class="grid grid-cols-1 gap-1 pt-3">
            ...
            <div class="col-span-1 pt-3 sm:col-span-1 lg:col-span-3">
              <label for="upload-image"
                ...
                @if($new_image)
                    <img src="{{ $new_image->temporaryUrl() }}" alt="" id="img-preview"
                class="rounded-1 absolute inset-2 left-1/2 -translate-x-1/2 transform text-center"
                style="height: -webkit-fill-available;">
                @else
                    @php
                        $image = $productImage->where('is_main', true)[0]->image;
                        @endphp
          ...
          <div class="grid grid-cols-1 gap-1 pt-3">
            ...
            <div class="flex w-full items-center justify-center">
              <label for="dropzone-file"
                ...
                @if ($new_images)
                <div class="rounded-1 absolute inset-2 left-1/2 -translate-x-1/2 transform text-center">
                    @foreach($new_images as $new_image)
                        <img src="{{ $new_image->temporaryUrl() }}" alt="" />
                    @endforeach
                </div>
                @else
                <div class="flex items-center justify-center rounded-1 absolute inset-2 left-1/2 -translate-x-1/2 transform text-center">
                    @php
                        $images = $productImage->where('is_main', false);
                    @endphp

                    @foreach($images as $image)
                        <img src="{{ asset('storage/' . $image->image) }}" width="200" alt="" >
                    @endforeach
                </div>
                @endif
