...
@section('content')
<x-page-title currentPage="Quản lý người dùng" pageTitle="Sửa thông tin nhân viên" />
            ...
            <div class="col-span-9 sm:col-span-4">
              <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6 sm:col-span-3">
                  ...
                  <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="input-form">
                  @error('name')
                ...
                <div class="col-span-6 sm:col-span-3">
                  ...
                  <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="input-form">
                  @error('email')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                  @enderror
                </div>
                ...
                <div class="col-span-6">
                  ...
                  <select class="input-form" name="role" id="role" autocomplete="role">
                    ...
                    @foreach ($roles as $role)
                      <option value="{{ $role->id }}" @if (old('role', $roleId) == $role->id) selected @endif>
                        {{ $role->name }}
                      </option>
                    @endforeach
                  </select>
                  ...
                  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    @if ($permissions)
                      @foreach ($permissions as $permission)
                        <x-buttons.toggle-button label="{{ $permission->name }}" value="{{ $permission->id }}" name="permissions[]" check="{{ $permission->checked }}" />
                      @endforeach
                    @endif
                  </div>
                </div>
