...
@section('content')
  <h3 class="text-3xl font-medium text-gray-700">Edit Product</h3>
        ...
        <div class="overflow-hidden shadow sm:rounded-md">
          ...
          <div class="bg-white px-4 py-5 sm:p-6">
            <div class="col-span-9 sm:col-span-4">
              <div class="grid grid-cols-6 sm:grid-cols-3 gap-6">
                <div class="col-span-6 sm:col-span-3">
                  <label for="name" class="my-2 block text-sm font-medium text-gray-700">Tến sản phẩm</label>
                  <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="input-form">
                  @error('name')
                ...
                <div class="col-span-6 sm:col-span-3">
                  ...
                  <input type="text" slug="slug" id="slug" value="{{ old('slug', $product->slug) }}" class="input-form">
                  @error('slug')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                  @enderror
                </div>
              ...
              <div class="grid grid-cols-6 sm:grid-cols-3 gap-6">
                ...
                <div class="col-span-6 sm:col-span-3">
                  ...
                  <input type="number" name="stock_qty" id="stock_qty" min="0" value="{{ old('stock_qty', $product->stock_qty) }}"
              ...
              <div class="grid grid-cols-6 sm:grid-cols-3 gap-6 py-3">
                <div class="col-span-6 sm:col-span-3">
                  <label for="regular_price" class="my-2 block text-sm font-medium text-gray-700">Giá thường</label>
                  <input type="number" name="regular_price" id="regular_price" value="{{ old('regular_price', $product->regular_price) }}"
                    class="input-form appearance-number-none">
                  @error('regular_price')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                  @enderror
                  ...
                  <label for="upload-image"
                    ...
                    <div class="flex flex-col items-center justify-center pb-6 pt-5">
                      ...
                      <p class="filename text-xs text-gray-500 dark:text-gray-400">SVG, PNG, JPG or
                        GIF
                        (MAX. 800x400px)</p>
                        <input  type="file" name="image" class="hidden" value="{{ old('image', $product->thumbnail) }}" id="upload-image" onchange="loadFile(event)"/>
                    </div>
                  </label>
                  @error('image')
                    <span class="text-sm text-red-500">{{ $message }}</span>
