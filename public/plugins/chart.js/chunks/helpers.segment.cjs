...
/**
 * Returns true if `value` is an array (including typed arrays), else returns
 * false.
 * @param value - The value to test.
 * @function
 ...
 * returns the value returned by `fn`. If `fn` is not a function, this method
 * returns undefined.
...
const keyResolvers = {
    // Chart.helpers.core resolveObjectKey should resolve empty key to root
    // object
    '': (v) => v,
    // default resolvers
    x: (o) => o.x,
...
// Adapted from
// https://stackoverflow.com/questions/31128855/comparing-ecma6-sets-for-equality#31129384
const setsEqual = (a, b) => {
    if (a.size !== b.size) {
        return false;
    }
...
/**
 * Implementation of the nice number algorithm used in determining where axis
 * labels will go
 ...
 * i.e. the number of digits after the decimal point, of the value of this
 * Number.
 * @param x - A number.
 * @returns The number of decimal places.
 * @private
...
function getAngleFromPoint(centrePoint, anglePoint) {
    ...
    if (angle < -0.5 * PI) {
        angle += TAU; // make sure the returned angle is in the range of
                      // (-PI/2, 3PI/2]
    }
    return {
        angle,
...
/**
 * Returns `start`, `end` or `(start + end) / 2` depending on `align`. Defaults
 * to `center`
 * @private
 */ const _alignStartEnd = (align, start, end) => align === 'start' ? start : align === 'end' ? end : (start + end) / 2;
/**
 * Returns `left`, `right` or `(left + right) / 2` depending on `align`.
 * Defaults to `left`
 * @private
    ...
    if (meta._sorted) {
        ...
        if (minDefined) {
            start = _limitValue(Math.min(// @ts-expect-error Need to type _parsed
                _lookupByKey(_parsed, iScale.axis, min).lo, // @ts-expect-error
                                                            // Need to fix
                                                            // types on
                                                            // _lookupByKey
        ...
        if (maxDefined) {
            count = _limitValue(Math.max(// @ts-expect-error Need to type _parsed
                _lookupByKey(_parsed, iScale.axis, max, true).hi + 1, // @ts-expect-error
                                                                      // Need
                                                                      // to fix
                                                                      // types
                                                                      // on
 ...
 * @returns The effective line height in pixels (size * 1.2 if value is
 *     invalid).
...
/**
 * Converts the given value into a TRBL corners object (similar with css
 * border-radius).
 ...
 * is called with `context` as first argument and the result becomes the new
 *     input.
...
function splineCurve(firstPoint, middlePoint, afterPoint, t) {
    ...
    // between points
    // http://scaledinnovation.com/analytics/splines/aboutSplines.html This
    // function must also respect "skipped" points
 ...
 * |splineCurve|, but preserves monotonicity of the provided data and ensures
 * no local extremums are added between the dataset discrete points due to the
 * interpolation. See :
...
function getContainerSize(canvas, width, height) {
    ...
    if (width === undefined || height === undefined) {
        ...
        if (!container) {
            ...
            const rect = container.getBoundingClientRect(); // this is the
                                                            // border box of
                                                            // the container
...
function getMaximumSize(canvas, bbWidth, bbHeight, aspectRatio) {
    ...
    if (width && !height) {
        ...
        // If the canvas has width, but no height, default to aspectRatio of 2
        // (canvas default)
 ...
 */ function retinaScale(chart, forceRatio, forceStyle) {
    ...
    // display size, making the chart visually bigger, so let's enforce it to
    // the "correct" values. See
 ...
 * pixels. That can happen in some cases where
 * `element` has a size relative to its parent and this last one is not yet
 * displayed, for example because of `display: none` on a parent node.
